...
{ 
	...
	while(1) {
		...
		DBGpthread_mutex_lock(&mutexLemuri,Plabel); 
		printf("Lemuri non possono piu' salire, ci sono %d lemuri a bordo\n", NLemuriSuIppopotamo);
...
{ 
	...
	while(1) {
		...
		while(LemuriPossonoSalireSuIppopotamo == 0 || NLemuriSuIppopotamo > 3){
			rc = pthread_cond_wait(&condIppoEntraInAcquaRivaNord, &mutexLemuri);
			if(rc) {
				printf("Problem in wait() at %s: %d", Plabel, rc);
				exit(-1);
			}
		}
		...
		rc = pthread_cond_wait(&condIppoEsceRivaSudProd, &mutexLemuri);
		if(rc) {
			printf("Problem in wait() at %s: %d", Plabel, rc);
			exit(-1);
		}
		...
		fflush(stdout);
		NLemuriSuIppopotamo--;
		rc = pthread_mutex_unlock(&mutexLemuri);
		if(rc) {
			printf("Problem in unlock() at %s: %d", Plabel, rc);
			exit(-1);
		}
