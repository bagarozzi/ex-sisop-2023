	...
	while(1) {
		...
		*/
		...
		while(numVecchiettiInLettura != 0){
			printf("Attacchino %s aspetta i vecchi\n", Alabel);
			numAttacchiniInAttesa++;
			pthread_cond_wait(&condAttacchini, &mutex);
			numAttacchiniInAttesa--;
		}
		...
		*/
		...
		numAttacchiniAlLavoro--;/*
		if(numAttacchiniAlLavoro == 1){
			printf("Attacchino %s ha finito per primo e va a fanculo\n", Alabel);
		}
		else{
			printf("Attacchino %s ha finito per secondo e lo segnala ai vecchi\n", Alabel);
			pthread_cond_broadcast(&condVecchietti);
		} */
		if(numAttacchiniAlLavoro == 0){
			printf("Entrambi gli attacchini hanno finito\n");
			pthread_cond_broadcast(&condVecchietti);
		}
		pthread_mutex_unlock(&mutex);
