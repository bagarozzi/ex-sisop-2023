...
int NumSoccorritoriFiniti=0;
pthread_cond_t condInizioSoccorso;
pthread_cond_t condFineCure;
...
{ 
	while(1) {
		...
		if(NumSoccorritoriArrivati ==  1){
			printf("soccorritore %s arriva per primo dal prof\n", Label );
		}
		...
		/* tempo per soccorrere, fino a qui è giusto */
		sleep(2);
	
		DBGpthread_mutex_lock(&mutex, Label); 
		printf("soccorritore %s finisce soccorso a prof \n", Label );
		fflush(stdout);
		/* 1) il soccorritore ha finito ma prima di andare via deve aspettare i che entrambi i soccorritori abbiano finito.
		   2) Inoltre, l'ultimo soccorritore che finisce dice al professore che lui puo' andare via */
		/* da completare */
		NumSoccorritoriFiniti++;

		fflush(stdout);
		NumSoccorritoriArrivati = 0;
		NumSoccorritoriFiniti = 0;
		DBGpthread_mutex_unlock(&mutex, Label); 
...
{ 
	...
	if(rc) PrintERROR_andExit(rc, "pthread_cond_init failed");
	rc = pthread_cond_init(&condFineCure, NULL);
	if(rc) PrintERROR_andExit(rc, "pthread_cond_init failed");
