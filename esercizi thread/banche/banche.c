/* file:  es2_banche.c */
/*
Osservazini prima di fare l'esercizio: 
    - Ogni banca è indipendente dall'altra -> ognuna ha la sua mutex
    - Per ogni banca, solo un thread può agire sui suoi conti alla volta
    - La Banca d'Italia è indipendente dalle altre ed agisce sui conti di tutte
*/
...
{ 
	...
	while(1) 
	{
        ...
        if(rc){
            PrintERROR_andExit(rc, "Locking failed");
        }
		/* FINE AGGIUNTA */
        ...
        if(rc){
            PrintERROR_andExit(rc, "Unlocking failed");
        }
...
{ 
	...
	while(1) 
	{
        ...
        for(i = 0; i < NUMBANCHE; i++){
            ...
            if(rc){
                PrintERROR_andExit(rc, "Locking failed in banca d'italia");
            }
        }
		...
		*/
        ...
        for(i = 0; i < NUMBANCHE; i++){
            ...
            if(rc){
                PrintERROR_andExit(rc, "Unlocking failed in banca d'italia");
            }
        }
