...
void *do_thread(void *p_index)
{
	...
	*/
	...
	{
		sleep(1);
		printf("thread index %d: thread_ID %u  ITERAZIONE %d\n", *((int*)p_index), (int)pthread_self(), i );
		i++;
...
int main()
{
	...
	/* uccido il pthread creato per secondo, indice 1 */
	printf("Termino il thread di indice 1 THREAD ID %u\n", (int)(vthreads[1]) );
	pthread_cancel(vthreads[1]);
	printf("non lo vedo piu nell output\n");
	fflush(stdout);
